...
title ESPI W01 - Freeze, ONNX Export, Benchmark (FIXED)
...
rem ====== 1) FREEZE ENV (no multiline python -c) ======
echo.
echo === [1/3] FREEZE ENVIRONMENT ===
"%PYTHON%" -V > "%PROJ%\environment_info.txt"
"%PYTHON%" -m pip freeze > "%PROJ%\requirements.txt"
"%PYTHON%" -c "import platform; print('Python:', platform.python_version()); print('Platform:', platform.platform())" >> "%PROJ%\environment_info.txt"
"%PYTHON%" -c "import torch; print('Torch:', torch.__version__); print('CUDA available:', torch.cuda.is_available())" >> "%PROJ%\environment_info.txt"
echo [OK] Wrote environment_info.txt and requirements.txt
echo.

rem ====== 2) EXPORT ONNX (avoid workers to prevent spawn/pagefile issues) ======
echo === [2/3] EXPORT ONNX ===
echo Running export via training script (resume auto, workers=0, bs=1)...
"%PYTHON%" "%TRAIN%" --device cpu --clean-root "%AVERAGED%" --output-dir "%ROOT%\outputs_W01" --epochs 1 --resume auto --workers 0 --batch-size 1 --tile 224 --overlap 32 --export-onnx "%ONNX%"
if errorlevel 1 (
  echo [WARN] Export returned non-zero exit code. Check logs above.
) else (
  echo [OK] Export completed (check ONNX exists).
)
echo.
...
mkdir "%TMP%"
echo Running benchmark on limit=30, tile=224, overlap=32 ...

if not exist "%ONNX%" (
  echo [WARN] ONNX file not found: "%ONNX%"
  echo       Will still run PyTorch part; ONNX will be skipped.
)
...
echo.
echo If you saw WinError 1455 (pagefile too small): increase Virtual Memory (System Properties > Advanced > Performance Settings > Advanced > Virtual memory).
pause
