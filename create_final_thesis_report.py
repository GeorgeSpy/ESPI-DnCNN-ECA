# C:\ESPI_DnCNN\create_final_thesis_report.py
# Create comprehensive thesis results report
import os, json, numpy as np, pandas as pd
from datetime import datetime

OUT_DIR = r"C:\ESPI_TEMP\RF_EVAL"
REPORT_PATH = r"C:\ESPI_TEMP\FINAL_THESIS_RESULTS_REPORT.md"
...
def create_thesis_report():
    ...
    # Load all results
    comparison = load_json(os.path.join(OUT_DIR, "compare_baseline_pattern_hybrid.json"))
    robustness_comp = load_json(os.path.join(OUT_DIR, "robustness_comparison.json"))
    lodo = load_json(os.path.join(OUT_DIR, "lodo_results.json"))
    lobo = load_json(os.path.join(OUT_DIR, "lobo_results.json"))
    negative = load_json(os.path.join(OUT_DIR, "negative_control.json"))
    stability = load_json(os.path.join(OUT_DIR, "feature_stability.json"))
    
    # Load feature lists
    with open(os.path.join(OUT_DIR, "feature_list_pattern.txt"), 'r') as f:
        pattern_features = f.read().strip().split('\n')
    with open(os.path.join(OUT_DIR, "feature_list_hybrid.txt"), 'r') as f:
        hybrid_features = f.read().strip().split('\n')
    
    # Calculate LODO/LOBO averages
    lodo_acc = np.mean([r['accuracy'] for r in lodo])
    lodo_f1 = np.mean([r['macro_f1'] for r in lodo])
    lobo_acc = np.mean([r['accuracy'] for r in lobo])
    lobo_f1 = np.mean([r['macro_f1'] for r in lobo])
    
    # Create report
    report = f"""# ESPI-DnCNN Enhanced Classification Results
...
- **Average accuracy:** {lodo_acc:.4f} ({lodo_acc*100:.1f}%)
- **Average macro-F1:** {lodo_f1:.4f} ({lodo_f1*100:.1f}%)
...
- **Average accuracy:** {lobo_acc:.4f} ({lobo_acc*100:.1f}%)
- **Average macro-F1:** {lobo_f1:.4f} ({lobo_f1*100:.1f}%)
...
- **Shuffled accuracy:** {negative['shuffled_accuracy']:.4f} ({negative['shuffled_accuracy']*100:.1f}%)
- **Expected random:** {negative['expected_random']:.4f} ({negative['expected_random']*100:.1f}%)
...
{chr(10).join([f"- {feat}" for feat in pattern_features])}

### Hybrid Features (22 total):
{chr(10).join([f"- {feat}" for feat in hybrid_features])}

### Feature Stability Analysis:
- **Cross-seed correlation:** {stability['stability_analysis']['avg_correlation']:.4f}
- **Consistently stable features:** {len(stability['stability_analysis']['consistently_stable'])}/16
- **Top stable features:** {', '.join(stability['stability_analysis']['consistently_stable'][:5])}
...
"""
    ...
    print(f"[OK] Comprehensive thesis report created: {REPORT_PATH}")
    return REPORT_PATH
