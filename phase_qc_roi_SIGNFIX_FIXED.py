...
-----------------------
QC για FFT-based phase extraction με προαιρετικό ROI και quality threshold,
**ανθεκτικό σε αντιστροφή πρόσημου** της φάσης (συνηθισμένο όταν επιλέγεται ο “λάθος” side-lobe).
Για κάθε frame υπολογίζει δύο φορές:
  A) resid = wrap( phi_w - wrap(+phi_u) )
  B) resid = wrap( phi_w - wrap(-phi_u) )
και κρατάει αυτό με το μικρότερο RMSE.

Features:
- Αναδρομικό scan σε outputs (PNG/NPY).
- Metrics μόνο σε valid pixels (ROI λευκό=keep, quality>=qmin keep).
- Σώζει CSV και (προαιρετικά) residual heatmaps από την “καλύτερη” επιλογή πρόσημου.

Usage:
python phase_qc_roi_SIGNFIX_FIXED.py ^
  --out-root "C:\...\band28" ^
