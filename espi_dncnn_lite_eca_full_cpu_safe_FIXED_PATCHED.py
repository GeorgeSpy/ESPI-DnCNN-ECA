...
def build_split(paths: List[Path], mode: str, val_ratio: float, seed: int, root: Optional[Path] = None,
                loio_group: Optional[str] = None) -> DataSplit:
    ...
    if mode == "random":
        ...
        if loio_group is None: loio_group = keys[-1]
...
def main(a: Args):
    ...
    # Model/Optim
    cfg = DnCNNLiteECAConfig(in_channels=1, out_channels=1, features=a.features, depth=a.depth, norm=a.norm,
                              eca_interval=a.eca_interval, eca_k=3, residual_weight=1.0,
                              spa_interval=a.spa_interval, spa_k=a.spa_ks)
...
def parse_args() -> Args:
    ...
    return Args(
        ...
        features=a.features, depth=a.depth, norm=a.norm, eca_interval=a.eca_interval, spa_interval=a.spa_interval,
        spa_ks=a.spa_ks, sigma_g=a.sigma_g, speckle=a.speckle, seed=a.seed, patience=a.patience, device=a.device,
        tensorboard=bool(a.tensorboard), export_onnx=(a.export_onnx if a.export_onnx else None),
        resume=(a.resume if a.resume else None), w_edge=a.w_edge
    )
