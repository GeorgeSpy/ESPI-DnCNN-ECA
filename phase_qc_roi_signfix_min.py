...
# QC for FFT phase extraction with optional ROI and quality threshold.
# Sign-robust: tries +phi_u and -phi_u per frame, keeps the lower RMSE.
...
def main():
    ...
    for rel in rels:
        ...
        rpos = resid_pos[valid]
        rneg = resid_neg[valid]
        apos = np.abs(rpos)
        aneg = np.abs(rneg)
        rmse_pos = float(np.sqrt(np.mean(rpos**2)))
        rmse_neg = float(np.sqrt(np.mean(rneg**2)))
