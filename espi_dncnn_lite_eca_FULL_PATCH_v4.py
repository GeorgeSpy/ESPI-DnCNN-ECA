...
# -*- coding: utf-8 -*-
"""
ESPI Denoising — DnCNN-Lite + ECA (+ optional Spatial Lite Attention) — FULL PATCH V4
...
- REAL evaluation (single-shot noisy vs averaged pseudo-clean) — periodic + final
...
class ECA(nn.Module):
    """Efficient Channel Attention with adaptive kernel, temperature and gain."""
    def __init__(self, channels: int, k_size: int = 0, temp: float = 0.75, gain: float = 0.5, centered: bool = True):
        super().__init__()
        ...
        self._last_gate_mean: Optional[float] = None  # diagnostic
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = self.gap(x)                                     # [B,C,1,1]
        y = y.squeeze(-1).transpose(1,2)                    # [B,1,C]
        g = self.sigmoid(self.conv1d(y) / max(self.temp, 1e-6))
        self._last_gate_mean = g.mean().item()
        g = g.transpose(1,2).unsqueeze(-1)                  # [B,C,1,1]
        if self.centered:
            scale = 1.0 + self.gain * (g - 0.5) * 2.0       # [1-gain, 1+gain]
        else:
            scale = g                                       # [0..1]
        return x * scale
...
class DnCNNLiteECA(nn.Module):
    def __init__(self, cfg: DnCNNLiteECAConfig):
        ...
        blocks, ecas, spas = [], [], []
        pos_set = set(cfg.eca_positions) if cfg.eca_positions is not None else None
        for i in range(cfg.depth - 2):
            ...
            if use_eca:
                ecas.append(ECA(c, k_size=cfg.eca_k, temp=cfg.eca_temp, gain=cfg.eca_gain, centered=cfg.eca_centered))
            else:
...
def main(a: Args):
    ...
    cfg = DnCNNLiteECAConfig(
        ...
        eca_centered=a.eca_centered, eca_positions=a.eca_positions,
    ...
    print(
        ...
        f"eca_k={a.eca_k} eca_temp={a.eca_temp} eca_gain={a.eca_gain} "
    ...
    for epoch in range(start_epoch + 1, start_epoch + a.epochs + 1):
        ...
        if eca_means:
            print("[ECA] gate means:", ", ".join(f"{v:.3f}" for v in eca_means))
...
def parse_args() -> Args:
    ...
    return Args(
        ...
        eca_interval=a.eca_interval, eca_k=a.eca_k, eca_temp=a.eca_temp, eca_gain=a.eca_gain,
        eca_centered=bool(a.eca_centered), eca_positions=parse_int_list(a.eca_positions),
