...
# -*- coding: utf-8 -*-
"""
CPU-safe ESPI Denoising — DnCNN-Lite + ECA (Single File)
...
try:
    # Prefer torch.amp (avoids deprecation warnings)
...
def run_validation_fullres(model, dl, criterion, device, tile, overlap, autocast_ctx):
    ...
    for noisy, clean in dl:
        ...
        # Ensure [B,1,H,W] for tiled inference
...
def main(a: Args):
    ...
    for epoch in range(1, a.epochs+1):
        ...
        print(f"Epoch {epoch:03d}/{a.epochs} | Train {tr:.4f} | Val {vl:.4f} | PSNR {vp:.2f} | SSIM {vs:.4f} | EdgeF1 {vf:.4f} | LR {lr_now:.2e}")
