#!/usr/bin/env python3
"""
fine_tune_paired_min.py
-----------------------
Minimal, self-contained script to fine-tune a DnCNN-Lite + ECA denoiser on *paired* ESPI data:
(noisy-root ↔ clean-root with identical filenames). CPU-safe by default (workers=0).

Usage (one line, CMD/PowerShell):
  python fine_tune_paired_min.py --noisy-root "C:\...\W01_ESPI_90db-PseudoNoisy" ^
    --clean-root "C:\...\W01_ESPI_90db-Averaged" ^
    --outdir     "C:\...\outputs_W01_paired" ^
    --epochs 20 --batch-size 2 --lr 5e-5 --crop 224 --workers 0 --device cpu

Optional: initialize from an existing checkpoint (partial load ok):
  --resume "C:\...\outputs_W01\checkpoints\best.pth"

Optional: import your exact model from a file to match saved weights:
  --model-def "C:\ESPI_DnCNN\espi_dncnn_lite_eca_full_cpu_safe_FIXED_PATCHED_v2.py" --model-class "DnCNNLiteECA"

If you don't pass --model-def/--model-class, a built-in DnCNNLiteECA is used.
"""

import argparse, math, random, time, importlib.util, types, sys, os
...
class PairedESPIDataset(Dataset):
    def __init__(self, noisy_root, clean_root, split="train", val_ratio=0.2, seed=42, ext=".png", crop=224, aug=False):
...
class ECALite(nn.Module):
    """Efficient Channel Attention (1D conv over channel descriptor)."""
...
def try_load_external_model(def_path: str, class_name: str, in_ch=1, out_ch=1, features=64, depth=17):
    """Try to import a model class from an external .py file and instantiate it."""
...
def ssim_torch(x, y, window=None, C1=0.01**2, C2=0.03**2):
    ...
    w = window.expand(x.size(1),1,window.size(0),window.size(1))  # [C,1,k,k] -> depthwise
...
class CombinedLoss(nn.Module):
    def __init__(self, w_mse=0.7, w_ssim=0.2, w_edge=0.1):
        super().__init__()
        self.w_mse = w_mse; self.w_ssim = w_ssim; self.w_edge = w_edge

    def forward(self, out, target):
        mse = F.mse_loss(out, target)
        ssim = ssim_torch(out, target)
        edge = F.l1_loss(sobel_grad(out), sobel_grad(target))
        return self.w_mse*mse + self.w_ssim*(1.0-ssim) + self.w_edge*edge, (mse.item(), ssim.item(), edge.item())
...
def main():
    ...
    # model args / external import
    ap.add_argument("--resume", type=str, default=None, help="checkpoint to warm-start from (best/last .pth)")
    ap.add_argument("--model-def", type=str, default=None, help="path to .py file with model class")
    ap.add_argument("--model-class", type=str, default=None, help="class name in that file")
    ...
    if args.resume and os.path.exists(args.resume):
        print(f"[RESUME] Loading checkpoint (partial OK): {args.resume}")
        ckpt = torch.load(args.resume, map_location="cpu")
        state = ckpt.get("model_state", ckpt if isinstance(ckpt, dict) else None)
        if state is None:
            print("[RESUME] Warning: no model_state found; trying raw state dict load")
            state = ckpt
        try:
            model.load_state_dict(state, strict=False)
        except Exception as e:
            # try to align common prefixes (e.g. 'module.' from DDP)
            new_state = {}
            for k,v in state.items():
                nk = k.replace("module.", "")
                new_state[nk] = v
            model.load_state_dict(new_state, strict=False)
        print("[RESUME] Loaded (strict=False).")

    model.to(device)

    # opt & loss
    opt = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-5)
    criterion = CombinedLoss(w_mse=0.7, w_ssim=0.2, w_edge=0.1)
    ...
    print("Tip: export to ONNX once satisfied with real-like performance.")
