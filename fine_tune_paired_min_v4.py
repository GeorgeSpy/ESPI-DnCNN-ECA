#!/usr/bin/env python3
# fine_tune_paired_min_v4.py
# - Robust padding before crop (never zero-sized tiles)
# - No 'verbose=' in ReduceLROnPlateau (compat)
# - Option to force batch_size=1 in .bat to sidestep heterogeneous sizes
...
def pad_to_min(x, min_h, min_w):
    h, w = x.shape
    pad_h = max(0, min_h - h); pad_w = max(0, min_w - w)
    if pad_h>0 or pad_w>0:
        x = np.pad(x, ((pad_h//2, pad_h - pad_h//2), (pad_w//2, pad_w - pad_w//2)), mode="reflect")
    return x
...
class PairedESPIDataset(Dataset):
    ...
    def _load_gray(self, p):
        try:
            ...
            # return a tiny dummy to be handled by padding
    ...
    def __getitem__(self, i):
        ...
        # final safeguard
        if noisy.shape != (self.crop, self.crop):
            noisy, clean = center_crop_pair(noisy, clean, self.crop)
...
def main():
    ...
    ap.add_argument("--batch-size", type=int, default=1)   # safer default
    ap.add_argument("--workers", type=int, default=0)
    ap.add_argument("--val-ratio", type=float, default=0.2)
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--device", default="cpu")
    ap.add_argument("--crop", type=int, default=192)       # safer default
