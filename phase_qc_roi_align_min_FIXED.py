...
def best_offset(phi_w: np.ndarray, phi_u_wrapped: np.ndarray, valid: np.ndarray) -> float:
    # Estimate c that best aligns phi_u_wrapped to phi_w (c in [-pi,pi)).
    # Using circular mean of (phi_w - phi_u_wrapped).
    diff = wrap_pi(phi_w - phi_u_wrapped)
    z = np.exp(1j * diff[valid])
    if z.size == 0:
        return 0.0
    c = float(np.angle(z.mean()))
    return c
...
def main():
    ...
    for rel in rels:
        ...
        for sgn in (+1.0, -1.0):
            phi_u_s = sgn * phi_u
            phi_u_wr = wrap_pi(phi_u_s)
            c = best_offset(phi_w, phi_u_wr, valid)  # in [-pi,pi)
            ...
            if (best_rmse is None) or (rmse < best_rmse):
                best_rmse = rmse
                best_sign = int(sgn)
                best_offset_val = float(c)
                best_resid = resid
                best_abs = a

        p_pi4 = float(np.mean(best_abs > (np.pi/4.0)))
        p_pi2 = float(np.mean(best_abs > (np.pi/2.0)))

        rows.append((name, n_valid, f"{best_rmse:.6f}", f"{p_pi4:.6f}", f"{p_pi2:.6f}", str(best_sign), f"{best_offset_val:.6f}"))
        n_frames += 1
        sum_rmse += best_rmse
        sum_p4 += p_pi4
        sum_p2 += p_pi2
        ...
        if args.save_maps:
            ...
            vals = np.clip(np.abs(best_resid)/np.pi, 0.0, 1.0)
            m[valid] = vals[valid]
