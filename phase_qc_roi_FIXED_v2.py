...
# -*- coding: utf-8 -*-
r"""
phase_qc_roi_FIXED_v2.py
------------------------
QC for FFT-based phase extraction with optional ROI mask and quality threshold.
- Scans outputs recursively (supports nested subfolders).
- Works with PNG or NPY for wrapped/unwrapped/quality.
- Computes on *valid* pixels only (ROI white=255 kept; black=0 ignored; quality>=qmin kept).
- Saves CSV and optional residual heatmaps.

Usage:
python phase_qc_roi_FIXED_v2.py ^
  --out-root "C:\...\band28" ^
...
def main():
    ...
    for rel in rels:
        name = rel.stem
        w_path = (w_base / rel)
        # choose unwrapped counterpart (prefer same type as wrapped)
        candidates = []
        if w_is_png:
        ...
        rows.append((name, n_valid, f"{rmse:.6f}", f"{p_pi4:.6f}", f"{p_pi2:.6f}"))
    ...
    print(f"[SUMMARY over {n_frames} image(s)] (valid pixels only)")
