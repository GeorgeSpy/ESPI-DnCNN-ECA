...
Usage:
python phase_qc_roi.py ^
  --out-root "C:\...\W01_PhaseOut_masked_comp\band28" ^
...
def main():
    ...
    for rel in rels:
        name = rel.stem
        try:
            phi_w = load_npy_or_png((wnpy / rel).with_suffix(".npy"), (wpng / rel).with_suffix(".png"), True)
            phi_u = load_npy_or_png((unpy / rel).with_suffix(".npy"), (upng / rel).with_suffix(".png"), True)
        ...
        if args.save_maps:
            ...
            m[valid] = np.clip(a/np.pi, 0.0, 1.0)[valid]
    ...
    with open(out_csv, "w", newline="", encoding="utf-8") as fo:
        import csv; csv.writer(fo).writerows(rows)
